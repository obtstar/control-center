...
系统由**一个独立控制中心仓库**与**多个代码仓库**组成。控制中心仅承载设计开发控制文档与任务编排配置（无代码实现）；代码仓库承载全部代码（平台实现 + 业务），通过 Git Worktree 多分支管理实现物理隔离执行。
...
│  ├────────────────────────────────────────────────────────────────────┤ │
│  │  任务编排配置（版本化）                                             │ │
│  │  orchestration: 瀑布状态机 · 排班 · Prompt · Skill · 自我升级配置   │ │
│  └────────────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────────┘
...
| 组成 | 职责 | 关键技术 |
|-----|------|---------|
| 控制中心仓库 | **独立仓库，无代码实现**：设计开发控制文档（概要/外部设计、需求）+ 任务编排配置；不被自身调度，不纳入 `repository` 注册表 | Git / 编排配置 |
| 平台代码仓库 | 平台实现代码：后端 `control-api`、前端 `control-web`、数据库 `control-db`，受多仓库管理 | Java Spring Boot / React + PrimeReact + Vite / MySQL |
| Web 管理端 | 人类入口，掌握审核闸门与合并决策权（control-web 代码仓库构建部署） | React + PrimeReact + Vite + Nginx |
| 后端服务 | 任务编排、状态机、排班、RAG、外部集成（control-api 代码仓库） | Java Spring Boot |
...
| 集成 | 用途 |
|-----|------|
| 控制中心仓库 Git | 概要设计/外部设计/需求集中管理 + 编排配置，MR + 审核变更 |
