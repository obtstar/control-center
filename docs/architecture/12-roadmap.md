...
| 阶段 | 周期 | 交付 |
|-----|------|------|
| **一期：基础平台** | 2-3 周 | 控制中心仓库（设计文档 + 编排配置 + registry 注册表）+ 代码仓库脚手架（`control-api` Go / `control-web` React / `control-db` 占位——SQLite DDL 内嵌 control-api）· 接入企业内 LiteLLM 代理 · 仓库 OpenAPI 集成 · KB 只读检索 · 工作记录与工作报告 |
